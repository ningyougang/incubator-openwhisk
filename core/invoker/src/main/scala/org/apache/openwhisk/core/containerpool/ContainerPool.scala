...
class ContainerPool(childFactory: ActorRefFactory => ActorRef,
  ...
  def prewarmContainer(exec: CodeExec[_], memoryLimit: ByteSize, ttl: Option[FiniteDuration]): Unit = {
    if (hasPoolSpaceFor(busyPool ++ freePool ++ prewarmedPool, prewarmStartingPool, memoryLimit)) {
      println(s"busyPool: ${memoryConsumptionOf(busyPool)}")
      println(s"freePool: ${memoryConsumptionOf(freePool)}")
      println(s"prewarmedPool: ${memoryConsumptionOf(prewarmedPool)}")
      println(s"prewarmStartingPool: ${memoryConsumptionOfPrewarmStartingPool}")
      println(s"total: ${memoryConsumptionOf(busyPool ++ freePool ++ prewarmedPool)}")
      println("============================================")
      ...
      println("===Cannot create prewarm container")
      logging.warn(
        this,
  ...
  def memoryConsumptionOfPrewarmStartingPool[A](): Long = {
    prewarmStartingPool.map(_._2._2.toMB).sum
  }
    ...
    prewarmedPool.toSeq
      ...
      .map {
        case (ref, data) =>
          ...
          freePool = freePool + (ref -> data)
          println(s"====takePrewarmContainer before: ${memoryConsumptionOf(prewarmedPool)}")
          prewarmedPool = prewarmedPool - ref
          println(s"====takePrewarmContainer after: ${memoryConsumptionOf(prewarmedPool)}")
